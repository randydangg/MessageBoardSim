...
public class NotesServer {
 ...
 //       Map<String, String> dictionary = new HashMap<String, String>();	//used to store client details 
        int clientNumber = 0;
		...
		try {
			...
			while (true) {
				...
				if (command.startsWith("POST")) {	//if client wants to POST
					...
					String color = splitCommand[5];	//make an exception for when color is invalid
					String message[] = {};
					int temp = 0;
					for (int i = 6; i < splitCommand.length; i++) {	//store message in array
