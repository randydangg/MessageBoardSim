...
//	    PrintWriter output;
		//might have to use data stream classes to better handle the long strings, only problem is that it keeps throwing an exception
...
//						output.writeUTF("Note is successfully posted!");
						output.writeUTF(note.x_pos + " " + note.color + " " + note.message);
					}
...
//								output.writeUTF("requesting color and reference!");
								for (int a = 0; a < NOTES.size(); a++) {
